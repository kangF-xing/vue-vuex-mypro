<template>
    <div id="cartcontent">
        ...
        <div id="place-an-order">
            ...
            <router-link tag="div" :to="{path:trueorfals==false?'/cart':'/payorder/'}"
             :class="{dise:trueorfals==false}" v-show="redactorbuy">
                下单
            </router-link>
            <router-link tag="div" @click.native="remove" v-show="!redactorbuy" to="" :class="{dise:trueorfals==false}">
                删除
            </router-link>
        </div>
    </div>
    ...
    methods:{
        ...
        Addclass(item,index){
            ...
            if(this.total==0){
                ...
                this.trueorfals=true
                this.redactorbuy==true?this.trueorfalsprice=true:this.trueorfalsprice=false
            }
            this.list.map(i=>{
                if(i.trueorfalse==false){
                    this.color=-1
        ...
        checkAll(){
            ...
            if(this.color==-1){
                ...
                this.trueorfals=false
                this.trueorfalsprice=true
                if(this.redactorbuy==false||this.redactorbuy==true)this.trueorfalsprice=false
            }
            localStorage.arr=JSON.stringify(this.list);
        },
        // 点击编辑
